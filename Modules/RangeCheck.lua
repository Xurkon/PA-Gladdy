...
local GetSpellInfo = GetSpellInfo  -- Use wrapped GetSpellInfo from GladdyCompat (not C_GetSpellInfo - retail API)
...
function RangeCheck.CheckRange(self)
    ...
    -- Ascension/3.3.5 compatibility: UnitPhaseReason is BFA 8.0+ API (phasing system doesn't exist)
    -- Check if function exists before calling, otherwise skip phasing check
    local isPhased = false
    if UnitPhaseReason then
        isPhased = UnitPhaseReason(button.unit)
    end

    if (not UnitIsConnected(button.unit) or isPhased) then
