...
-- Ascension/3.3.5 compatibility: Use native functions, not C_* tables (retail API)
...
-- POLLING SYSTEM (Ascension Compatibility)

---------------------------

-- PollCastingState: Check if unit started/stopped casting (since events don't fire on Ascension)
function Castbar.PollCastingState(castBar)
    if not castBar or not castBar.unit then return end

    -- Don't poll in test mode
    if Castbar.test then return end

    -- Only poll when unit exists
    local unit = castBar.unit
    if not UnitExists(unit) then return end

    -- Ascension compatibility: UnitCastingInfo returns values in DIFFERENT ORDER than standard 3.3.5
    -- Standard: name, text, texture, startTime, endTime, isTradeSkill, castID, notInterruptible, spellId
    -- Ascension: name, text, displayName, texture, startTime, endTime, isTradeSkill, castID, notInterruptible
    -- The key difference: texture path is in position 4 instead of 3
    local v1, v2, v3, v4, v5, v6, v7, v8, v9 = UnitCastingInfo(unit)

    -- Detect which API we're using by checking if v4 is a string path (contains "Interface\")
    local name, texture, startTime, endTime, castID, notInterruptible
    if v4 and type(v4) == "string" and v4:find("Interface\\") then
        -- Ascension order: v1=name, v2=text, v3=displayName, v4=texture, v5=startTime, v6=endTime, v7=isTradeSkill, v8=castID, v9=notInterruptible
        name = v1
        texture = v4
        startTime = v5
        endTime = v6
        ...
        -- Standard 3.3.5 order: v1=name, v2=text, v3=texture, v4=startTime, v5=endTime, v6=isTradeSkill, v7=castID, v8=notInterruptible, v9=spellId
        name = v1
        texture = v3
        startTime = v4
        endTime = v5
    ...
    if name then
        -- Unit is casting
        -- Use castID + endTime to detect new casts (handles repeated casts of same spell)
        local currentCastIdentifier = tostring(castID) .. "_" .. tostring(endTime)
        if castBar.lastCastIdentifier ~= currentCastIdentifier then
            -- New cast detected! Trigger cast start manually
            castBar.lastCastIdentifier = currentCastIdentifier
            castBar.lastCastID = castID
            castBar.lastChannelID = nil -- Clear channel state

            -- Validate times are numbers
            if type(startTime) ~= "number" or type(endTime) ~= "number" then
                -- Fallback: can't calculate timing, skip
                return
            end

            -- Manually trigger cast start
            if castBar.spark then
                castBar.spark:Show()
            end
            castBar.value = (GetTime() - (startTime / 1000))
            ...
            castBar.channeling = nil
            castBar.fadeOut = nil
            Castbar:CAST_START(unit, name, texture, castBar.value, castBar.maxValue, notInterruptible)
        end
    else
        -- Not casting - check if we were casting before (cast ended)
        if castBar.lastCastID and castBar.casting then
            -- Cast ended successfully
            castBar.lastCastID = nil
            castBar.lastCastIdentifier = nil
            if castBar.spark then
                castBar.spark:Hide()
            end
            castBar.casting = nil
            ...
            castBar.fadeOut = true
            castBar.holdTime = GetTime() + 0.25
            Castbar:CAST_STOP(unit, 0, 1, 0, 1)
        end
    end

    -- Check for active channel (if not casting)
    if not name then
        local c1, c2, c3, c4, c5, c6, c7, c8 = UnitChannelInfo(unit)

        -- Detect Ascension order for channels (same logic as casts)
        local channelName, channelTexture, channelStartTime, channelEndTime, channelNotInterruptible
        if c4 and type(c4) == "string" and c4:find("Interface\\") then
            -- Ascension order
            channelName = c1
            channelTexture = c4
            channelStartTime = c5
            channelEndTime = c6
            channelNotInterruptible = c8
        else
            -- Standard order
            channelName = c1
            channelTexture = c3
            channelStartTime = c4
            channelEndTime = c5
            channelNotInterruptible = c7
        end

        if channelName then
            -- Unit is channeling
            -- Use channelName + endTime to detect new channels (handles repeated channels of same spell)
            -- This is critical for short channels like Penance that can be cast multiple times
            local currentChannelIdentifier = channelName .. "_" .. tostring(channelEndTime)
            if castBar.lastChannelIdentifier ~= currentChannelIdentifier then
                -- New channel detected! Trigger channel start manually
                castBar.lastChannelIdentifier = currentChannelIdentifier
                castBar.lastChannelID = channelName
                castBar.lastCastID = nil -- Clear cast state
                castBar.lastCastIdentifier = nil

                -- Validate times are numbers
                if type(channelStartTime) ~= "number" or type(channelEndTime) ~= "number" then
                    -- Fallback: can't calculate timing, skip
                    return
                end

                -- Manually trigger channel start
                if castBar.spark then
                    castBar.spark:Show()
                end
                castBar.value = (channelEndTime / 1000) - GetTime()
                castBar.maxValue = (channelEndTime - channelStartTime) / 1000
                castBar.holdTime = 0
                castBar.casting = nil
                castBar.channeling = true
                castBar.fadeOut = nil
                Castbar:CAST_START(unit, channelName, channelTexture, castBar.value, castBar.maxValue, channelNotInterruptible)
            end
        else
            -- Not channeling - check if we were channeling before (channel ended)
            if castBar.lastChannelID and castBar.channeling then
                -- Channel ended
                castBar.lastChannelID = nil
                castBar.lastChannelIdentifier = nil
                if castBar.spark then
                    castBar.spark:Hide()
                end
                castBar.channeling = nil
...
function Castbar:JOINED_ARENA()
    if Gladdy.db.castBarEnabled then
        for i=1, Gladdy.curBracket do
            ...
            -- Ascension/3.3.5 compatibility: UNIT_SPELLCAST_* events don't exist in Ascension
            -- Use polling system instead (OnUpdate checks for casts every frame)
            -- Events are registered for manual interrupts/test mode compatibility
            ...
            -- Polling system: Check for casts every frame
            castBar:SetScript("OnUpdate", function(self, elapsed)
                Castbar.OnUpdate(self, elapsed)
                -- Poll for cast/channel state changes (Ascension compatibility)
                Castbar.PollCastingState(self)
            end)
