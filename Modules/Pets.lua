    ...
    if Gladdy.db.petEnabled then
        ...
        self:SetScript("OnEvent", function(_, event, unitId)
            if event == "UNIT_PET" then
                Pets:CheckUnitPet(unitId)
            elseif event == "UNIT_PORTRAIT_UPDATE" then
                local petFrame = self.frames[unitId]
                if petFrame then
                    SetPortraitTexture(petFrame.healthBar.portrait, unitId)
                end
            end
        end)
    end
...
local function PetHealthOnUpdate(self)
...
function Pets:PET_STEALTH(unit)
    if Gladdy.db.petEnabled then
        ...
        healthBar.hp:SetStatusBarColor(0.66, 0.66, 0.66, 1)
        healthBar:SetScript("OnUpdate", nil)
    end
end

function Pets:CheckUnitPet(unitId, owner)
    local unit = owner == false and unitId or string_gsub(unitId, "%d$", "pet%1")
    local petFrame = self.frames[unit]
    if (not petFrame) then
        return
    end
    ...
    if guid then
        ...
        petFrame.healthBar.hp:SetMinMaxValues(0, healthMax)
        petFrame.healthBar.hp:SetValue(health)
        Pets:SetHealthText(petFrame.healthBar, health, healthMax)
        SetPortraitTexture(petFrame.healthBar.portrait, unit)
        petFrame.healthBar.hp:SetStatusBarColor(Gladdy:SetColor(Gladdy.db.petHealthBarColor))
        petFrame.healthBar:SetScript("OnUpdate", PetHealthOnUpdate)
    else
        petFrame.healthBar:SetAlpha(0)
        petFrame.healthBar:SetScript("OnUpdate", nil)
    end
