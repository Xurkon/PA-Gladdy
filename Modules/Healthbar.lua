...
function Healthbar.OnEvent(self, event, unit)
    -- Ascension/3.3.5 compatibility: RegisterUnitEvent doesn't exist in 3.3.5 (added in Legion 7.0)
    -- Filter events manually - only process if unit matches this healthBar's unit
    if unit ~= self.unit then return end

    local button = Gladdy.buttons[unit]
    if not button then return end  -- Not an arena unit (e.g., nameplate3, player)
    if (event == "UNIT_HEALTH" and not button.destroyed) or event == "UNIT_MAXHEALTH" then
        ...
        -- Ascension compatibility: UnitHealth may return nil or 0 initially
        -- Ignore invalid values, keep previous health values
...
function Healthbar:SetHealthStatusBarColor(unit, health, healthMax)
    ...
    -- Ascension compatibility: button.class may be empty string initially
    if button.class and button.class ~= "" and RAID_CLASS_COLORS[button.class] and Gladdy.db.healthBarClassColored then
        healthBar.hp:SetStatusBarColor(
...
function Healthbar:ENEMY_SPOTTED(unit)
    ...
    if UnitExists(unit) then
        ...
        -- Ascension compatibility: Use defaults if UnitHealth returns nil/0
