...
function Auras:CreateFrame(unit)
	...
	-- SetDrawEdge is Cataclysm 4.0+ API
	if auraFrame.cooldown.SetDrawEdge then
...
function Auras:CreateInterrupt(unit)
	...
	-- SetDrawEdge is Cataclysm 4.0+ API
	if interruptFrame.cooldown.SetDrawEdge then
...
function Auras:SPELL_INTERRUPT(unit,spellID,spellName,spellSchool,extraSpellId,extraSpellName,extraSpellSchool)
	...
	-- Ascension/3.3.5 compatibility: Check if interrupt exists in the table
	-- Some Ascension custom spells might not be in the interrupt database
	local interruptData = Gladdy:GetInterrupts()[spellName]
	if not interruptData then
		-- Unknown interrupt spell, ignore it
		return
	end

	local interruptSpellID = tostring(interruptData.spellID)
	if not Gladdy.db.auraListInterrupts[interruptSpellID] or not Gladdy.db.auraListInterrupts[interruptSpellID].enabled then
		return
	end
	...
	local duration = interruptData.duration * multiplier
	...
	interruptFrame.icon:SetTexture(interruptData.texture)
