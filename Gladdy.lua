...
function Gladdy:PixelPerfectScale(update)
	...
	if self.db and self.db.pixelPerfect and self.frame then
		-- SetIgnoreParentScale is a Legion+ API, not available in 3.3.5
		if self.frame.SetIgnoreParentScale then
...
function Gladdy:OnInitialize()
	self:Print("Gladdy v" .. self.version_num .. " - Backported to Ascension By Hutsh")
	...
	-- DEBUG: Check if Constants loaded properly
	if not self.GetSpecBuffs then
		self:Error("[CRITICAL] GetSpecBuffs is nil - Constants_Wrath.lua may not have loaded!")
	end
	if not self.GetSpecSpells then
		self:Error("[CRITICAL] GetSpecSpells is nil - Constants_Wrath.lua may not have loaded!")
	end
