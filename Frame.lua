...
function Gladdy:CreateMover(frame, xConfig, yConfig, name, points, width, height, xOffset, yOffset, activated)
    if not frame.mover then
        ...
        frame.mover:RegisterForDrag("LeftButton")
        frame.mover.dragRegistered = true

        frame.mover:SetScript("OnDragStart", function(self)
            self.point = { frame:GetPoint() }
            self.start = { frame:GetCenter() }
            frame:StartMoving()
            self:StartMoving()
        end)
        frame.mover:SetScript("OnDragStop", function(self)
        ...
        frame.mover:SetHeight(height or frame:GetHeight())
        frame.mover:SetWidth(width or frame:GetWidth())
        -- Ensure drag is registered (for movers created before this fix)
        if not frame.mover.dragRegistered then
            frame.mover:RegisterForDrag("LeftButton")
            frame.mover.dragRegistered = true
        end
    end
